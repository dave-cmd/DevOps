...
resource "aws_route_table" "public_route_table" {
  ...
  route {
    cidr_block = var.public-route-table-cidr_block
...
resource "aws_subnet" "private_data_subnet_az1" {
  vpc_id                  = aws_vpc.vpc.id
  cidr_block              = var.private-data-subnet-cidr_block-az1
  availability_zone       = data.aws_availability_zones.available_zones.names[4]
  map_public_ip_on_launch = false

  tags = {
    Name = "private-data-subent-az1"
  }
}
...
resource "aws_subnet" "private_data_subnet_az2" {
  vpc_id                  = aws_vpc.vpc.id
  cidr_block              = var.private-data-subnet-cidr_block-az2
